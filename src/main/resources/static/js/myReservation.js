    ...
    $('#edit-reservation-button').on('click', function() {
        ...
        if (selectedReservation) {
            ...
            if (checkInDate < currentDate) {
                alert($("#check-in-passed").text());
            } else if (selectedReservation.canceled) {
                alert($("#reservation-has-been-canceled").text());
            } else {
            if(language=="zh"){
                // Modalı göster
                                $('#cancellationPolicyModalZh').modal('show');

                                // Modal kapandığında yönlendirme yapmak için event listener ekleyin
                                $('#cancellationPolicyModalZh').on('hidden.bs.modal', function () {
                ...
                // Modalı göster
                                ...
                                // Modal kapandığında yönlendirme yapmak için event listener ekleyin
                                $('#cancellationPolicyModal').on('hidden.bs.modal', function () {
                                    const url = 'http://localhost:8080/bookNow?reservationId=' + selectedReservation.reservationId;
                                    window.location.href = url;
                                });
            }
...
function getReservationData() {
    ...
    $.ajax({
        url: "/api/reservation/myReservation",
        type: "get",
        dataType: "json",
